...
Gossiperl.Client.Supervisor.prototype.read = function(digestType, binDigest, digestInfo) {
  return (new Gossiperl.Client.Serialization.Serializer()).deserializeArbitrary( digestType, binDigest, digestInfo );
}
...
/**
 * Deserialize arbitrary digest.
 * 
 * @param {Array} binDigest - array of bytes containing an envelope
 * @param {Array} digestInfo - array of digest data fields
 * 
 * Each field should contain following properties:
 *  - name: name of the field
 *  - value: value prototype to read
 *  - id: numeric field id, starting from 0
 * The value does not matter as long as it is of an expected type.
 * For numeric types, a type_hint can be added; should be one of:
 *  - byte
 *  - i16
 *  - i32
 ...
 */
Gossiperl.Client.Serialization.Serializer.prototype.deserializeArbitrary = function(digestType, binDigest, digestInfo) {
  ...
  var transport = Gossiperl.Client.Util.getThriftTransport();
  var protocol  = new Thrift.TBinaryProtocol( transport );
  protocol = Gossiperl.Client.Util.setBufferOnProtocol(protocol, buf);
  var result = [];
  protocol.readStructBegin();
  for (var i=0; i<digestInfo.length; i++) {
    
    var thriftFieldInfo = protocol.readFieldBegin();
    var field = this.getFidData( thriftFieldInfo.fid, digestInfo );
    ...
    if ( field != null ) {
      ...
      if ( thriftFieldInfo.ftype === fieldType ) {
        switch (fieldType) {
          case Thrift.Type.BYTE:
            result[ field.name ] = protocol.readByte();
            break;
          case Thrift.Type.BOOL:
            result[ field.name ] = protocol.readBool();
            break;
          case Thrift.Type.I16:
            result[ field.name ] = protocol.readI16();
            break
          case Thrift.Type.I32:
            result[ field.name ] = protocol.readI32();
            break
          case Thrift.Type.I64:
            result[ field.name ] = protocol.readI64();
            break
          case Thrift.Type.DOUBLE:
            result[ field.name ] = protocol.readDouble();
            break
          case Thrift.Type.STRING:
            result[ field.name ] = protocol.readString();
            break;
          default:
            protocol.skip( thriftFieldInfo.ftype );
        }
      } else {
        protocol.skip( thriftFieldInfo.ftype );
      }
...
Gossiperl.Client.Serialization.Serializer.prototype.getFidData = function(fid, digestInfo) {
  for ( var i=0; i<digestInfo.length; i++ ) {
    if (digestInfo[i].id === fid) {
      return digestInfo[i];
    }
  }
  return null;
